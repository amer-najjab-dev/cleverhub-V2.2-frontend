...
interface PaymentObligationsProps {
  supplierId: string;
}

export const PaymentObligations = ({ supplierId }: PaymentObligationsProps) => {
  ...
  const loadObligations = async () => {
    try {
      const response = await deliveryService.getSupplierObligations(supplierId);
  ...
  if (loading) return <div className="text-center py-8">Cargando obligaciones...</div>;

  if (obligations.length === 0) {
    return <div className="text-center py-8 text-gray-500">No hay obligaciones de pago para este proveedor</div>;
  }
