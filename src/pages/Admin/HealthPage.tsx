    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Semáforo de Salud</h1>
            <p className="text-gray-600 mt-1">Monitoreo de estado de las farmacias</p>
          </div>
          <button
            onClick={loadHealth}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 mb-1">Estado Verde</p>
                <p className="text-3xl font-bold text-green-600">{health?.summary.green || 0}</p>
                <p className="text-xs text-gray-400 mt-1">Activas + ventas recientes</p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 mb-1">Estado Amarillo</p>
                <p className="text-3xl font-bold text-yellow-600">{health?.summary.yellow || 0}</p>
                <p className="text-xs text-gray-400 mt-1">Vencen pronto o sin actividad</p>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500 mb-1">Estado Rojo</p>
                <p className="text-3xl font-bold text-red-600">{health?.summary.red || 0}</p>
                <p className="text-xs text-gray-400 mt-1">Suspendidas o sin actividad</p>
              </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h2 className="font-semibold text-gray-900">
              {selectedStatus === 'green' && '🟢 Farmacias en Estado Verde'}
              {selectedStatus === 'yellow' && '🟡 Farmacias en Estado Amarillo'}
              {selectedStatus === 'red' && '🔴 Farmacias en Estado Rojo'}
            </h2>
          </div>
          <div className="divide-y divide-gray-100">
            {currentData.length === 0 ? (
              <p className="p-6 text-center text-gray-500">No hay farmacias en este estado</p>
            ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">Licencia: {pharmacy.license}</p>
                        ...
                        {pharmacy.total_sales !== undefined && (
                          <p className="text-sm text-gray-500">
                            Ventas totales: {pharmacy.total_sales}
                          </p>
                        )}
                      </div>
                    </div>
