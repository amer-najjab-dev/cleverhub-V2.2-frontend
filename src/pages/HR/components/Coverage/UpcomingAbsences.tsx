...
const getTypeLabel = (type: string) => {
  switch (type) {
    ...
    case 'maternity': return 'Baja maternidad';
    case 'paternity': return 'Baja paternidad';
    default: return type;
  }
};
  ...
  if (absences.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm p-4">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Ausencias 
próximas</h3>
...
próximas</h3>
      <div className="space-y-2">
        {absences.map(absence => (
          <div key={absence.id} className="flex items-start gap-2 p-2 border 
...
rounded-full ${getTypeColor(absence.type)}`}>
