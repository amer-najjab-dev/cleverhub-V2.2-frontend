...
import { employeeService, Employee, Shift } from 
  ...
  const handleSubmit = async () => {
    try {
      const data = {
        ...
        defaultShiftId: formData.defaultShiftId ? 
parseInt(formData.defaultShiftId) : undefined,
      ...
      setFormData({ email: '', fullName: '', phone: '', dni: '', password: '', 
defaultShiftId: '', vacationDays: 25 });
    } catch (error: any) {
...
border-blue-600"></div>
        ...
        <div className="text-center py-8 text-gray-500">No hay empleados 
registrados</div>
      ) : (
...
text-gray-900">{emp.user?.full_name}</div>
                  <div className="text-sm text-gray-500">{emp.user?.email}</div>
                  <div className="text-xs text-gray-400 mt-1">
                    Turno: {emp.default_shift?.name || 'No asignado'} | 
...
rounded-lg"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 
mb-1">Turno por defecto</label>
                <select
                  value={formData.defaultShiftId}
                  onChange={(e) => setFormData({ ...formData, defaultShiftId: 
e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                >
...
value={shift.id}>{shift.name}</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 
mb-1">Días de vacaciones</label>
                <input
                  type="number"
                  value={formData.vacationDays}
                  onChange={(e) => setFormData({ ...formData, vacationDays: 
