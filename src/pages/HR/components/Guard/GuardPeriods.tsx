...
import { employeeService, GuardPeriod, Shift } from 
...
endDate: '' });
  ...
  const loadData = async () => {
    try {
      ...
      setShifts(shiftsData.filter(s => s.isGuard));
    } catch (error) {
...
hover:bg-blue-700 flex items-center gap-2"
      ...
      {periods.length === 0 ? (
        ...
        <div className="divide-y divide-gray-100">
          {periods.map(period => (
            <div key={period.id} className="p-4 flex items-center 
justify-between hover:bg-gray-50">
              <div>
                <div className="font-medium 
text-gray-900">{period.shiftName}</div>
                <div className="text-sm text-gray-500">
                  {new Date(period.startDate).toLocaleDateString('es-ES')} - 
{new Date(period.endDate).toLocaleDateString('es-ES')}
                </div>
              </div>
              <button onClick={() => handleDelete(period.id)} className="p-1 
